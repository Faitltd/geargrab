<script lang="ts">
  ...
  let verificationStatus: UserVerificationStatus  = null;
  let loading = true;
  ...
  let authState = simpleAuth.authState;

  onMount(async () => {
    if ($authState.user) {
      try {
        verificationStatus = await verificationService.getUserVerificationStatus($authState.user.uid);
      } catch (error) {
  ...
  async function startEmailVerification() {
    ...
    try {
      await verificationService.verifyEmail($authState.user.uid, email);
  ...
  async function startPhoneVerification() {
    if (!$authState.user || !phoneNumber) return;

    try {
      const result = await verificationService.verifyPhone($authState.user.uid, phoneNumber);
      if (result.success) {
  ...
  function getVerificationLevelColor(level) {
    switch (level) {
  ...
  function getVerificationLevelBg(level) {
    switch (level) {
  ...
  async function confirmPhoneVerification() {
    if (!$authState.user || !verificationCode) return;

    try {
      const result = await verificationService.confirmPhoneVerification($authState.user.uid, verificationCode);
      if (result.success) {
  ...
  function handleBackgroundCheckSubmitted(event) {
    const { requestId, provider, checkType } = event.detail;
    console.log('Background check submitted:', { requestId, provider, checkType });

    // Reload verification status
    if ($authState.user) {
      verificationService.getUserVerificationStatus($authState.user.uid).then(status => {
  ...
  function isBackgroundCheckRequired(level: 'basic' | 'standard' | 'premium') {
