<script lang="ts">
  // BRAND NEW Social-Only Signup Modal - v3.0.0 - NO EMAIL/PASSWORD
  ...
  async function handleSocialSignup(provider: 'google' | 'apple' | 'facebook' | 'github') {
    loading = true;
    error = '';
  ...
  >
    ...
    >
      ...
      <div class="text-center mb-8">
        <h2 id="signup-title" class="text-2xl font-bold text-white mb-2">Join GearGrab</h2>
        <p class="text-gray-300">Sign up with your preferred method</p>
      </div>
      ...
      <div class="space-y-3">
        ...
        <!-- Apple Signup -->
        <button
          on:click={() => handleSocialSignup('apple')}
          disabled={loading}
        ...
        >
          ...
          {loading ? 'Creating account...' : 'Continue with Apple'}
        </button>

        <!-- Facebook Signup -->
        <button
          on:click={() => handleSocialSignup('facebook')}
          disabled={loading}
