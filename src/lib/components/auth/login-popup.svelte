<script lang="ts">
  ...
  async function handleGoogleLogin() {
    console.log('🔐 Login popup: Starting Google login...');
    loading = true;
    errors = {};

    try {
      console.log('🔄 Login popup: Calling simpleAuth.signInWithGoogle...');
      const result = await simpleAuth.signInWithGoogle();

      if (result.success) {
        console.log('✅ Login popup: Google sign-in successful');
