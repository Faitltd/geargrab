<script>
  ...
  export let buttonSize = 'default'; // 'small', 'default', 'large'
  ...
  function getProviderIcon(provider) {
    // Return SVG icons for each provider
    const icons = {
...
{#if showTitle}
  <div class="text-center mb-6">
    <h3 class="text-lg font-medium text-gray-900 mb-2">Sign in with your account</h3>
    <p class="text-sm text-gray-600">Choose your preferred sign-in method</p>
  </div>
{/if}
  ...
  {#each filteredProviders as provider}
    ...
    <button
      ...
      class="
        ...
        {layout === 'compact' ? 'flex-col space-y-1' : 'space-x-3'}
      "
    >
      {#if loading[provider]}
      ...
      <span class="{layout === 'compact' ? 'text-xs' : ''}">
        {layout === 'compact' ? config.name : `Continue with ${config.name}`}
      </span>
    </button>
  {/each}
</div>
