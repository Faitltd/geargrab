<script lang="ts">
  ...
  export let animation: 'fade-up' | 'fade-left' | 'fade-right' | 'scale-in' | 'slide-down' = 'scale-in';
  export let threshold: number = 0.1;
  export let className: string = '';
  export let startOffset: number = 0; // Additional offset from when element enters viewport
  export let endOffset: number = 0.5; // How far through viewport before animation completes
  export let reverseOnScrollUp: boolean = true;
  export let smoothness: number = 0.1; // Animation smoothness (0-1, lower = smoother)
  ...
  function applyAnimation(progress: number) {
    if (!element) return;
