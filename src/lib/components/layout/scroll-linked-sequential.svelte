<script lang="ts">
  ...
  export let baseDelay: number = 0; // Base delay as a percentage (0-1)
  export let incrementDelay: number = 0.1; // Delay increment between items (0-1)
  ...
  function applyAnimation(element: HTMLElement, progress: number) {
    if (!element) return;
