...
import type { Listing, Booking } from '$lib/types/firestore';
...
export async function createPaymentIntent(
  ...
  try {
    ...
    if (testMode) {
      ...
      console.log('🧪 Test mode enabled for payment intent');
    }
...
export function calculateBookingFees(
  ...
  guaranteeTier?: 'basic' | 'standard' | 'premium' | 'none'
): {
  days: number;
  rentalFee: number;
  serviceFee: number;
  ...
  // Calculate GearGrab Guarantee fee
  let guaranteeFee = 0;
  if (guaranteeTier === 'basic') {
    guaranteeFee = 5;
  } else if (guaranteeTier === 'standard') {
    guaranteeFee = 10;
  } else if (guaranteeTier === 'premium') {
    guaranteeFee = 15;
  }
  ...
  const totalPrice = rentalFee + serviceFee + deliveryFee + guaranteeFee;
...
async function createBooking(bookingData: Omit<Booking, 'id'>): Promise<string> {
  if (!adminFirestore) {
    throw new Error('Firebase Admin is not configured for booking creation.');
  }

  const now = new Date();
  const docRef = await adminFirestore.collection('bookings').add({
    ...bookingData,
    createdAt: bookingData.createdAt || now,
    updatedAt: bookingData.updatedAt || now
  });
...
export async function createBookingWithPayment(
  ...
  // Calculate fees
  const fees = calculateBookingFees(listing, startDate, endDate, deliveryMethod, guaranteeTier);
  ...
  const bookingData: Omit<Booking, 'id' | 'createdAt' | 'updatedAt'> = {
    ...
    deliveryDetails,
    guaranteeTier,
    guaranteeCost: fees.guaranteeFee,
...
export function calculateOwnerPayout(booking: Booking): number {
  // Owner gets 85% of the rental fee (excluding service fee, guarantee, etc.)
  const rentalFee = booking.totalPrice - (booking.guaranteeCost || 0);
